...
follows [Semantic Versioning](https://semver.org/). **Extending a calendar's coverage is a minor
release; correcting a date that was wrong is a patch.** See
[Versioning](README.md#versioning).

## [1.0.0] — 2026-08-14

Same package as `0.1.0`, released again under a version range consumers can actually follow.

### Changed

- **`1.0.0` rather than `0.2.0`, four hours after the first release.** Below `1.0.0`, npm's caret
  pins the minor: `npm install market-hours` writes `^0.1.0`, which accepts `0.1.9` but not
  `0.2.0`. Calendar refreshes ship as minors, so every default install would have been frozen out
  of every refresh — and since queries past the verified horizon throw rather than guess, those
  installs would eventually have started failing with no route for the fix to reach them. Reported
  from a real integration before anyone was depending on it.
- **The versioning policy now splits calendar changes by kind.** Extending coverage is a minor,
  because it is additive — dates that used to throw now answer. Correcting a date that was wrong is
  a patch, because the previous answer was wrong and the fix should reach people quickly. The old
  policy made every calendar change a minor, which does not survive contact with `^0.x` ranges.
- Releases publish through npm trusted publishing, so no token exists to leak and every tarball
  after this one carries a provenance attestation generated by the workflow that built it.

### Fixed

- The CommonJS build no longer uses `moduleResolution: node10`, removed in TypeScript 7. Emitted
  output is unchanged: no `.d.ts` differs, and `dist/cjs` still emits CommonJS.
...
[1.0.0]: https://github.com/tickerfinance/market-hours/releases/tag/v1.0.0
